...
# defines the facial landmark detector
...
def run_dlib_shape(image):
    ...
    dlibout = np.reshape(np.transpose(face_shapes[:, np.argmax(face_areas)]), 136) #[68, 2]

    return dlibout, resized_image

# Library for obtaining the features
def extract_features_labels():
    """
    This funtion extracts the landmarks features for all images in the folder 'dataset/images'.
    ...
    #labels = {line.split(',')[0] : int(line.split(',')[4]) for line in lines[2:]}
    labels = {line.split(',')[0]: [int(line.split(',')[col]) for col in range(1, 6)] for line in lines[2:]}
